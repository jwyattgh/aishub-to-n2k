...
test('Signal K delta uses the server\'s own AIS paths', () => {
  const d = n2k.delta(CLASS_A, 'aishub-to-n2k', 0)
  assert.strictEqual(d.context, 'vessels.urn:mrn:imo:mmsi:311000123')
  assert.strictEqual(d.updates[0].source.label, 'aishub-to-n2k')
  assert.strictEqual(d.updates[0].timestamp, '2026-09-24T01:26:02.000Z')
  const byPath = {}
  d.updates[0].values.forEach(v => { byPath[v.path] = byPath[v.path] ? Object.assign(byPath[v.path], v.value) : v.value })
  assert.deepStrictEqual(byPath['navigation.position'], { latitude: 18.3, longitude: -67.5 })
  assert.strictEqual(byPath[''].name, 'BIG SHIP')
  assert.strictEqual(byPath[''].mmsi, '311000123')
  assert.strictEqual(byPath[''].communication.callsignVhf, 'C6XX9')
  assert.strictEqual(byPath['sensors.ais.class'], 'A')
  assert.strictEqual(byPath['navigation.state'], 'motoring')
  assert.deepStrictEqual(byPath['design.length'], { overall: 150 })
  assert.deepStrictEqual(byPath['design.aisShipType'], { id: 70, name: 'Cargo ship' })
  assert.ok(Math.abs(byPath['navigation.speedOverGround'] - 12 * 0.514444) < 0.001)
  assert.strictEqual(byPath['navigation.destination.commonName'], 'SAN JUAN')
  const b = n2k.delta(CLASS_B, 'x', 0)
  const paths = b.updates[0].values.map(v => v.path)
  assert.ok(!paths.includes('navigation.headingTrue'), 'unknown heading is left out')
  assert.ok(!paths.includes('navigation.state'), 'undefined nav status is left out')
})
